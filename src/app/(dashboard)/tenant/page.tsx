...
export default function TenantPortalPage() {
  ...
  async function handleInitiateMpesa(event: React.FormEvent) {
    ...
    try {
      ...
      if (res.status === "pending") {
        void pollMpesaStatus(res.paymentId, amountNumber);
      }
    } catch (err) {
      const message =
  ...
  async function pollMpesaStatus(paymentId: string, paidAmount: number) {
    let attempts = 0;
      ...
      try {
        ...
        if (res.status === "success" || res.status === "failed") {
          ...
          if (res.status === "success") {
            setConfirmation({ paymentId, amount: paidAmount });
            void refetch();
          }
          return;
        }
  ...
  function handleDismissConfirmation() {
    ...
    setMpesaError(null);
    setMpesaAmount(latestInvoice?.amount != null ? String(latestInvoice.amount) : "");
    setMpesaPhone("");
  }
